<?php 
  ...
  if (isset($_POST['updatestudent'])) {
  	...
  	if (!empty($_FILES['photo']['name'])) {
	  	 ...
	  	 $photo = explode('.', $photo);
		 $photo = end($photo); 
		 $photo = $studentID.date('Y-m-d-m-s').'.'.$photo;
  	}else{
  		$photo = $oldPhoto;
  	}
  	

  	$query = "UPDATE `student_info` SET `name`='$name',`studentID`='$studentID',`class`='$class',`city`='$address',`pcontact`='$pcontact',`photo`='$photo' WHERE `id`= $id";
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
	...
	<?php
		if (isset($id)) {
			$query = "SELECT `id`, `name`, `studentID`, `class`, `city`, `pcontact`, `photo`, `datetime` FROM `student_info` WHERE `id`=$id";
	...
	<form enctype="multipart/form-data" method="POST" action="">
		<div class="form-group">
	  	...
	  	<div class="form-group">
		    ...
		    <input name="studentID" type="text" class="form-control" pattern="[0-9]{6}" id="studentID" value="<?php echo $row['studentID']; ?>" required="">
	  	</div>
	  	<div class="form-group">
	  	...
	  	<div class="form-group">
		    ...
		    <input name="pcontact" type="text" class="form-control" id="pcontact" value="<?php echo $row['pcontact']; ?>" pattern="[0-9]{3}[0-9]{3}[0-9]{4}" placeholder="01........." required="">
	  	</div>
	  	<div class="form-group">
